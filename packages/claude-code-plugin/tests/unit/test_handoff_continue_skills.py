...
- /continue retrieves those notes by the `handoff` tag via list-by-recency
  and enumerates candidates (description + date) rather than consuming the
  top match.

If the skills are rewritten, these guard against silently dropping the
tag convention that couples the two skills together.
"""
...
def test_continue_retrieves_by_tag_and_enumerates_candidates() -> None:
    ...
    # The candidate list is the point — it must offer choices with a date,
    # not narrate the single top match.
