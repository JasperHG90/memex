...
4. **Procedure recall**: for "how do I X?" queries, check the `<scope>:procedure:*` KV keys first:
   - `memex_kv_list(prefix="global:procedure:")` for global procedures; `memex_kv_list(prefix="project:<id>:procedure:")` for project-scoped → `memex_kv_get(key)` for active value
