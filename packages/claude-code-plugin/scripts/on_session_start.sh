...
trap 'rm -f "$tmp_vaults" "$tmp_kv" "$tmp_notes" "$tmp_project_vault"' EXIT

kv_key="claude-code:vault:${project_id}"
# URL-encode the key using jq
encoded_key=$(jq -rn --arg k "$kv_key" '$k | @uri')

curl -sf --max-time 5 "${API}/vaults" > "$tmp_vaults" 2>/dev/null &
curl -sf --max-time 5 "${API}/kv" > "$tmp_kv" 2>/dev/null &
curl -sf --max-time 5 "${API}/notes?sort=-created_at&limit=10" > "$tmp_notes" 2>/dev/null &
curl -sf --max-time 5 "${API}/kv/${encoded_key}" > "$tmp_project_vault" 2>/dev/null &
wait
...
# --- Parse project vault ---
project_vault=""
if [ -s "$tmp_project_vault" ]; then
    project_vault=$(jq -r '.value // empty' "$tmp_project_vault" 2>/dev/null) || true
fi

vault_context_md=""
if [ -n "$project_vault" ]; then
    vault_context_md="**Active vault for this project**: \`${project_vault}\` (from KV key \`${kv_key}\`). Pass \`vault_id: \"${project_vault}\"\` on all write operations (\`memex_add_note\`, \`memex_kv_write\`)."
fi
...
session_context=""
for part in "$vaults_md" "$vault_context_md" "$kv_md" "$notes_md"; do
    if [ -n "$part" ]; then
