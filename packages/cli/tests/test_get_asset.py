"""Tests for the `memex note assets get` command."""
...
def test_get_asset_to_file(runner, mock_api, mock_config, monkeypatch, tmp_path):
    ...
    result = runner.invoke(app, ['assets', 'get', asset_path, '-o', str(out_file)], obj=mock_config)
...
def test_get_asset_to_stdout(runner, mock_api, mock_config, monkeypatch):
    ...
    result = runner.invoke(app, ['assets', 'get', asset_path], obj=mock_config)
