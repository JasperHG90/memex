...
MEMEX_KV_WRITE_DESC = (
    ...
    '\n'
    'Required: value (str), key (str). Key MUST match the namespace regex: '
    ...
    '"default to…". Namespace mapping by scope cue:\n'
    '- "about me / for myself / I prefer X (no qualifier)" → `user:<field>`\n'
    '- "in this repo / project / codebase" → `project:<id>:<field>`\n'
    '- "whenever I use <app>" → `app:<app-id>:<field>` (NOT `user:<app>:…`)\n'
    '- "across our projects / company-wide" → `global:<field>`\n'
    'Full rules + edge cases live in the agent system prompt (§KV_NAMESPACE).\n'
    '\n'
    ...
    'history envelope — each write appends; prior versions queryable via '
    'memex_kv_get(include_history=true).\n'
    '\n'
    'Deletion is CLI-only (`memex kv delete`); do NOT attempt deletion.'
)
