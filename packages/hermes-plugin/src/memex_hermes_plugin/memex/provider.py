...
# File name (under HERMES_HOME) for spilling the pending-queue to disk at
# exit when the fire-and-forget drain has not finished. Loaded and replayed
# on the next provider initialisation so no transcript chunk is lost across
# process restarts.
...
class MemexMemoryProvider(MemoryProvider):
    ...
    def __init__(self) -> None:
        ...
        # Background drain thread (fire-and-forget). None when idle.
        self._bg_drain_thread: threading.Thread | None = None
        # Replay any writes spilled to disk by a previous process that
        # exited before the drain could finish.
        self._load_spill()
    ...
    def initialize(self, session_id: str, **kwargs: Any) -> None:  # type: ignore[override]
        ...
        # Replay any writes spilled to disk by a previous process.
        self._load_spill()
    ...
    def shutdown(self) -> None:  # type: ignore[override]
        ...
        if self._api is not None:
            ...
            # Fire-and-forget drain may not complete before the process
            # exits; spill any still-pending writes so they can be
    ...
    def _drain_pending(self) -> None:
        ...
        Spawns a daemon thread that runs ``_drain_pending_sync`` (the
        synchronous drain loop) and returns immediately. Callers on
        latency-sensitive paths â ``on_pre_compress``, ``on_session_end``,
        ``sync_turn`` flushes â no longer block on network I/O or the
        120s note-row wait. The daemon thread is reaped at process exit;
        any still-pending writes are spilled to disk by ``shutdown`` so
        they can be replayed on the next start.
        """
        if self._api is None or self._config is None:
            return
        with self._state_lock:
            if self._bg_drain_thread is not None and self._bg_drain_thread.is_alive():
                return  # A drain is already running.
            if not self._pending:
                return
            t = threading.Thread(target=self._drain_pending_sync, name='memex-drain', daemon=True)
            self._bg_drain_thread = t
        t.start()

    def _drain_pending_sync(self) -> None:
        """Process the pending queue FIFO (synchronous worker).
        ...
        in-flight ingest to materialise. Because this runs on a daemon
        thread, that wait no longer blocks the caller.
        """
        if self._api is None or self._config is None:
            return
    ...
    def _spill_to_disk(self) -> None:
        ...
        Called from ``shutdown`` after the fire-and-forget drain has been
        launched. The daemon thread may not finish before the interpreter
        tears down; spilled writes are replayed by ``_load_spill`` on the
        next ``initialize``.
        """
        ...
        if not pending:
            try:
                (self._hermes_home / _SPILL_FILE_NAME).unlink(missing_ok=True)
            except Exception:
                pass
            return
        serialisable = []
        for entry in pending:
            e = dict(entry)
            if 'append_id' in e and not isinstance(e['append_id'], str):
                e['append_id'] = str(e['append_id'])
            serialisable.append(e)
        try:
            path = self._hermes_home / _SPILL_FILE_NAME
    ...
    def _load_spill(self) -> None:
        """Replay writes spilled by a previous process at startup."""
        if self._hermes_home is None:
            return
        path = self._hermes_home / _SPILL_FILE_NAME
        if not path.exists():
        ...
        with self._state_lock:
            for entry in data:
                if not isinstance(entry, dict):
                    continue
                e = dict(entry)
                if 'append_id' in e and isinstance(e['append_id'], str):
                    try:
                        e['append_id'] = UUID(e['append_id'])
                    except ValueError:
                        continue
                if len(self._pending) >= _PENDING_MAX:
                    break
        ...
        try:
            ...
            pass
        logger.info('Replayed %d spilled session-note writes from %s', len(data), path)
