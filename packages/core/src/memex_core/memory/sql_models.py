...
class TokenUsage(SQLModel, table=True):  # type: ignore
    """
    Log of token usage for LLM interactions.
    Persists usage metrics per session and model for observability and cost tracking.
    Also used for transient tracking and aggregation of token usage.
    """
    ...
    session_id: str = Field(
        ...
        index=True,
        nullable=False,
        description='The session identifier (request-scoped or global) for this interaction.',
    )
    models: list[str] = Field(
        default=[],
        sa_column=Column(ARRAY(Text), server_default=sql_text("'{}'::text[]")),
        description='The names of the LLM models used (e.g. gemini-3-pro).',
    )
    ...
    cost: float | None = Field(
        default=None,
        description='Estimated cost of the call in USD, if pricing is available.',
    )
    ...
    context_metadata: dict[str, Any] = Field(
        ...
        sa_column=Column('context_metadata', JSONB, server_default=sql_text("'{}'::jsonb")),
        description='JSON metadata linking to specific operations, documents, or entities.',
    )
    ...
    def __add__(self, other: 'TokenUsage') -> 'TokenUsage':
        return TokenUsage(
            ...
            models=list(set(self.models + other.models)),
            cost=(self.cost or 0.0) + (other.cost or 0.0),
            is_cached=self.is_cached and other.is_cached,
        )
    ...
    def __iadd__(self, other: 'TokenUsage') -> 'TokenUsage':
        ...
        self.models = list(set(self.models + other.models))
        self.cost = (self.cost or 0.0) + (other.cost or 0.0)
        self.is_cached = self.is_cached and other.is_cached
        return self
