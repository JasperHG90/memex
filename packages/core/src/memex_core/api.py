...
class MemexAPI:
    ...
    async def propose_event_cases(self, *, limit: int = 20, min_events: int = 3) -> list[UUID]:
        """§8 / §18.6.2 EVENT-cluster case auto-promotion.

        Find worked-episode candidate notes — content-vault notes
        (``role != 'case'``, active) whose extraction produced
        ``>= min_events`` EVENT memory units — and file a lint proposal
        (``target_type='note'``, ``set_note_role`` pre-selected) for each,
        as DRAFTS requiring confirmation (never silently promoted, §8).
        Pending-dedup prevents re-nagging. Returns note ids proposed.

        Clustering note: §8 sketches 'clusters of EVENT units sharing a task
        context + outcome'. As-built proxy — a single note whose extraction
        yielded a cluster of ``>= min_events`` events IS a worked episode. A
        cross-note clustering pass is a documented future refinement.
        """
        ...
        candidates: list[tuple[UUID, UUID, str, int]] = []
        async with self.metastore.session() as session:
            stmt = (
                select(MemoryUnit.note_id, func.count(col(MemoryUnit.id)).label('n'))
                .where(col(MemoryUnit.fact_type) == 'event')
                .where(col(MemoryUnit.note_id).is_not(None))
                .group_by(col(MemoryUnit.note_id))
                .having(func.count(col(MemoryUnit.id)) >= min_events)
                .limit(limit * 3)
            )
            for note_id, n in (await session.exec(stmt)).all():
                note = await session.get(Note, note_id)
                if note is None or note.role == 'case' or note.status != 'active':
                    continue
                vault = await session.get(Vault, note.vault_id)
                vkind = (
                    vault.kind.value
                    if vault is not None and hasattr(vault.kind, 'value')
                    else (str(vault.kind) if vault is not None else None)
                )
                if vault is None or vkind == 'system':
                    continue
                title = note.title or (note.original_text or '')[:80]
                candidates.append((note_id, note.vault_id, title, int(n)))
                if len(candidates) >= limit:
                    break

        proposed: list[UUID] = []
        for note_id, vid, title, n in candidates:
            try:
                req = ExternalProposalRequest(
                    rule_name='event_case_promotion',
                    lint_type='governance',
                    target_type='note',
                    target_id=str(note_id),
                    description=f'Worked-episode candidate ({n} events): {str(title)[:120]}',
                    suggested_action=(
                        'Confirm to promote this note to a case (role=case) via set_note_role.'
                    ),
                    vault_id=str(vid),
                    evidence={'event_unit_count': n},
                    proposed_action={'action_name': 'set_note_role', 'params': {'role': 'case'}},
                )
                status, _ = await insert_external_proposal(
                    self, req, vault_id=vid, actor='system:event_case'
                )
                if status == 'created':
                    proposed.append(note_id)
            except Exception:
                logger.exception('event-case proposal failed for note %s', note_id)
        return proposed
