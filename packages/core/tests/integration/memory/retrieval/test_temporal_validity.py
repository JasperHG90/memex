...
class TestTemporalValidity:
    ...
    async def test_as_of_exactly_equals_valid_to_excludes(
        self, session: AsyncSession, engine_instance, embedder
    ):
        """as_of == valid_to should EXCLUDE (half-open interval [valid_from, valid_to)).

        Tests the SQL predicate directly on EntityCooccurrence to avoid
        interference from 1st-order graph results.
        """
        from sqlmodel import select as sq_select

        e1 = await self._create_entity(session, f'BndTo-A-{uuid4().hex[:6]}')
        e2 = await self._create_entity(session, f'BndTo-B-{uuid4().hex[:6]}')
        ...
        # Query with as_of == valid_to (should return 0 rows)
        stmt_at = (
            sq_select(EntityCooccurrence)
            .where(EntityCooccurrence.entity_id_1 == id1)
            .where(EntityCooccurrence.entity_id_2 == id2)
            .where(
                or_(
                    col(EntityCooccurrence.valid_from).is_(None),
                    col(EntityCooccurrence.valid_from) <= boundary,
                )
            )
            .where(
                or_(
                    col(EntityCooccurrence.valid_to).is_(None),
                    col(EntityCooccurrence.valid_to) > boundary,
                )
            )
        )
        result_at = await session.exec(stmt_at)
        rows_at = result_at.all()

        # Query with as_of one second before valid_to (should return 1 row)
        before = datetime(2024, 5, 31, 23, 59, 59, tzinfo=timezone.utc)
        stmt_before = (
            sq_select(EntityCooccurrence)
            .where(EntityCooccurrence.entity_id_1 == id1)
            .where(EntityCooccurrence.entity_id_2 == id2)
            .where(
                or_(
                    col(EntityCooccurrence.valid_from).is_(None),
                    col(EntityCooccurrence.valid_from) <= before,
                )
            )
            .where(
                or_(
                    col(EntityCooccurrence.valid_to).is_(None),
                    col(EntityCooccurrence.valid_to) > before,
                )
            )
        )
        result_before = await session.exec(stmt_before)
        rows_before = result_before.all()

        assert len(rows_at) == 0, (
            f'as_of == valid_to should exclude the cooccurrence, got {len(rows_at)} rows'
        )
        assert len(rows_before) == 1, (
            f'as_of < valid_to should include the cooccurrence, got {len(rows_before)} rows'
        )

    async def test_as_of_exactly_equals_valid_from_includes(
        self, session: AsyncSession, engine_instance, embedder
    ):
        """as_of == valid_from should INCLUDE (closed lower bound).

        Tests the SQL predicate directly on EntityCooccurrence.
        """
        from sqlmodel import select as sq_select

        e1 = await self._create_entity(session, f'BndFrom-A-{uuid4().hex[:6]}')
        e2 = await self._create_entity(session, f'BndFrom-B-{uuid4().hex[:6]}')
        ...
        # Query with as_of == valid_from (should return 1 row)
        stmt_at = (
            sq_select(EntityCooccurrence)
            .where(EntityCooccurrence.entity_id_1 == id1)
            .where(EntityCooccurrence.entity_id_2 == id2)
        ...
        # Query with as_of one second before valid_from (should return 0 rows)
        ...
        assert len(rows_at) == 1, (
            f'as_of == valid_from should include the cooccurrence, got {len(rows_at)} rows'
        )
        assert len(rows_before) == 0, (
            f'as_of < valid_from should exclude the cooccurrence, got {len(rows_before)} rows'
        )
