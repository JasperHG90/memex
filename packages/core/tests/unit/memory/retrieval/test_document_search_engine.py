...
@pytest.fixture
def engine(mock_embedder: MagicMock) -> DocumentSearchEngine:
    return DocumentSearchEngine(embedder=mock_embedder)
...
class TestDocumentSearchEngine:
    ...
    def test_init_default(self, mock_embedder: MagicMock) -> None:
        engine = DocumentSearchEngine(embedder=mock_embedder)
    ...
    def test_init_with_ner(self, mock_embedder: MagicMock) -> None:
        ner = MagicMock()
        engine = DocumentSearchEngine(embedder=mock_embedder, ner_model=ner)
        assert engine.graph_strategy.ner_model is ner

    def test_init_with_lm(self, mock_embedder: MagicMock) -> None:
        lm = MagicMock()
        engine = DocumentSearchEngine(embedder=mock_embedder, lm=lm)
...
class TestMultiQueryFusion:
    ...
    def test_single_batch_passthrough(self) -> None:
        ...
        result = DocumentSearchEngine._fuse_multi_query(batches, limit=10)
        assert len(result) == 2
