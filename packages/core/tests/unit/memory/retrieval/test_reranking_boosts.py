...
class TestRecencyBoost:
    ...
    def test_recent_unit_gets_boost_above_one(self) -> None:
    ...
    def test_old_unit_gets_boost_below_one(self) -> None:
    ...
    def test_very_old_unit_floors_at_point_one(self) -> None:
    ...
    def test_missing_event_date_neutral_boost(self) -> None:
    ...
    def test_future_event_date_clamped_to_one(self) -> None:
...
class TestTemporalProximityBoost:
    ...
    def test_high_temporal_proximity_boosts(self) -> None:
    ...
    def test_low_temporal_proximity_reduces(self) -> None:
    ...
    def test_missing_temporal_proximity_neutral(self) -> None:
...
class TestAlphaZeroDisablesBoosts:
    ...
    def test_alpha_zero_recent_unit(self) -> None:
    ...
    def test_alpha_zero_old_unit(self) -> None:
    ...
    def test_alpha_zero_preserves_original_order(self) -> None:
...
class TestCombinedBoosts:
    ...
    def test_boosts_can_change_ranking_order(self) -> None:
    ...
    def test_boost_bounds_with_default_alpha(self) -> None:
        """With default alpha=0.2, boosts should be bounded in [0.9, 1.1]."""
        # Minimum recency (0.1): boost = 1 + 0.2*(0.1-0.5) = 0.92
        # Maximum recency (1.0): boost = 1 + 0.2*(1.0-0.5) = 1.1
        # Minimum temporal (0.0): boost = 1 + 0.2*(0.0-0.5) = 0.9
        # Maximum temporal (1.0): boost = 1 + 0.2*(1.0-0.5) = 1.1

        # The worst case combined is 0.92 * 0.9 = 0.828
        # The best case combined is 1.1 * 1.1 = 1.21
        now = datetime.now(timezone.utc)

        # Best case
        best_unit = _make_unit(event_date=now, temporal_proximity=1.0)
        engine = _make_engine([0.0])
        engine._rerank_results('query', [best_unit])
        ...
        engine2._rerank_results('query', [worst_unit])


class TestEdgeCases:
    """Edge case tests for reranking boosts."""

    def test_empty_results_no_error(self) -> None:
        """Empty results list should return empty without error."""
        engine = _make_engine([])
        result = engine._rerank_results('query', [])
        assert result == []

    def test_no_reranker_returns_as_is(self) -> None:
    ...
    def test_min_score_filtering_still_works(self) -> None:
        ...
        result = engine._rerank_results('query', [unit_high, unit_low], min_score=0.5)
        assert len(result) == 1
        assert result[0] is unit_high

    def test_temporal_proximity_attribute_used(self) -> None:
