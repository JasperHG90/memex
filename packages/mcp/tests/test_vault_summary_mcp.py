...
# ── memex_regenerate_vault_summary ──


@pytest.mark.asyncio
async def test_regenerate_vault_summary(mock_api, mock_config, mcp_client):
    summary = _make_summary(TEST_VAULT_UUID)
    mock_api.vault_summary = AsyncMock()
    mock_api.vault_summary.regenerate_summary.return_value = summary

    result = await mcp_client.call_tool('memex_regenerate_vault_summary', {})
    data = parse_tool_result(result)
...
@pytest.mark.asyncio
async def test_regenerate_vault_summary_with_vault_id(mock_api, mock_config, mcp_client):
    vid = uuid4()
    summary = _make_summary(vid)
    mock_api.vault_summary = AsyncMock()
    mock_api.vault_summary.regenerate_summary.return_value = summary
    ...
    result = await mcp_client.call_tool('memex_regenerate_vault_summary', {'vault_id': str(vid)})
    data = parse_tool_result(result)

    assert data['vault_id'] == str(vid)
    mock_api.vault_summary.regenerate_summary.assert_called_once_with(vid)


@pytest.mark.asyncio
async def test_regenerate_vault_summary_error(mock_api, mock_config, mcp_client):
    mock_api.vault_summary = AsyncMock()
    mock_api.vault_summary.regenerate_summary.side_effect = RuntimeError('LLM failed')

    with pytest.raises(ToolError, match='LLM failed'):
        await mcp_client.call_tool('memex_regenerate_vault_summary', {})
