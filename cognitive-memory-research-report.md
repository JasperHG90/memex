...
**Status:** *Implemented in PR (2026-05-03) — wave-16/F11-decay-scoring. Composition is a no-op at ship time (`decay_alpha = 0.0`); flip to a non-zero value in a follow-on config commit gated on the before/after benchmark. F40's `fsfm_branch_enabled` default flipped to `True` in the same PR — the FSFM SQL clause now activates for every retrieval, with the branch-level `COALESCE(..., FALSE)` wrap keeping cold-start (NULL-column) rows unfiltered.*
...
**2. New code location:**
- `packages/core/src/memex_core/memory/retrieval/decay.py:45` — module exporting `compute_decay_boost(unit, decay_alpha, now)` analogous to `compute_mw_boost` in `services/outcomes.py:46`. Closed-form `boost = 1.0 + decay_alpha × (importance × exp(-elapsed_days / stability) − 0.5)`. `now` is an injected parameter (matching F35's `compute_mw_ema_score(..., now)` signature pattern), not a bare in-function `now()` call — keeps the function deterministic under unit tests and lets the reranker share one timestamp across all per-unit boosts in a query. **NULL-handling contract** is a *split guard*, not a single early-return — each NULL input maps to a different semantic: `importance is None` → return `1.0` (genuine missing classification, no boost signal); `last_outcome_at is None` → return `1.0` (no temporal anchor for decay); `stability is None` → treat the decay term as `1.0` (the `stability → ∞` limit — permanent units get the importance-based lift `1.0 + decay_alpha × (importance − 0.5)` rather than being silently exempted from the composition). Permanent units are *importance-lifted, not decay-suppressed*. The only synthetic value is `stability=None → ∞`, which is mathematically the closed-form limit, not a guess — never substitute synthetic defaults for `importance` or `last_outcome_at`. Tests cover all three NULL cases independently in `tests/unit/memory/retrieval/test_decay.py`. Mirrors F40's branch-level `COALESCE(..., FALSE)` pattern at the boost-result level.
- `packages/core/src/memex_core/memory/retrieval/constants.py` — single source of truth for `STABILITY_SECONDS_PER_DAY = 86400.0`, `STABILITY_THRESHOLD = 0.10`, `INTENT_STABILITY_DAYS`, and `INTENT_IMPORTANCE`. Both F40's SQL builder and F11's Python boost import from here. F40's SQL builder binds the constants via `bindparams(...)` (asyncpg `$N` placeholder), never f-string interpolation — pinning test in `tests/unit/memory/retrieval/test_f40_sql_param_binding.py` asserts the rendered SQL is invariant under monkeypatched constant values.
- `packages/core/src/memex_core/alembic/versions/032_fsfm_decay_columns.py` — migration adding `importance: float | None`, `stability: float | None`, `last_outcome_at: datetime | None` to `MemoryUnit`. Existing rows get importance from F25's classifier output (`intent_class` → default importance map: permanent=1.0, durable=0.7, ephemeral=0.3); rows with NULL `intent_class` keep `importance = NULL` (no synthetic default). Stability from a per-class table (permanent=NULL meaning ∞, durable=180 days, ephemeral=14 days). **`last_outcome_at` is left NULL on backfill** for every existing row — this preserves the NULL-handling contract ("no temporal anchor → neutral boost"). The first `record_outcome` call transitions a unit from `last_outcome_at = NULL` (no decay) to `last_outcome_at = now()` (decay clock starts), matching the lifecycle of a brand-new post-migration unit. Backfilling to `created_at` would synthesise an age-based penalty on never-touched units; NULL is the correct invariant.

**3. Code adapted:**
- `packages/core/src/memex_core/memory/sql_models.py` (`MemoryUnit`) — append `importance` / `stability` / `last_outcome_at` columns alongside the existing FSRS revisit columns. Constants for the intent→stability map live in `memex_core.memory.retrieval.constants` so F40's FSFM SQL clause and F11's Python boost share one source of truth.
- `packages/core/src/memex_core/memory/retrieval/engine.py:1495-1496` — sixth multiplicative `decay_boost` factor in the rerank chain (`ce_score × recency_boost × temporal_boost × mw_boost × confidence_boost × decay_boost`). The composition site is the same `boosted_scores.append(...)` line F1c and F47 already extend. The per-query `now = datetime.now(timezone.utc)` snapshot already present for recency/temporal/mw is reused — every per-unit decay boost in a query shares one reference time.
- `packages/common/src/memex_common/config.py` — new config knob `decay_alpha` (default `0.0` at ship time — composition is a no-op until the post-merge benchmark passes; flip to `0.3` to match `mw_alpha` and `recency_alpha`'s shape, per BACKLOG F11 ship-time guard). `fsfm_branch_enabled` default also flipped from `False` to `True` in the same PR — separate commit, independently revertable. Three independent kill-switches keep the rollback story granular (column migration / `decay_alpha` / `fsfm_branch_enabled`).
- `packages/core/src/memex_core/services/outcomes.py:186-200` — `record_outcome` bumps `last_outcome_at = now()` on every counter update so decay is reset by recent behavioral signal. The same single-`now` snapshot used for the audit log row is passed to the SQL `UPDATE`.
...
- F40 (Tier-A latency-optimization) FSFM clause is now active: `apply_pre_filter=True` queries prune long-stale low-importance units pre-reranker. F40 originally shipped the SQL inert; F11's PR flips `fsfm_branch_enabled` to True as a *separate config commit within the same PR* — the column migration and the FSFM-active flag are two independent revertible commits.
...
**Status:** Implemented in PR for `wave-18/F22-edge-confidence-variance` (2026-05-03). Migration `033_confidence_evidence_count` ships the column + backfill INERTLY; F47 boost is bit-for-bit unchanged at ship time (`certainty_modulation_enabled = False` default).
