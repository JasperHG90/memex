...
**2. New code location:**
- `packages/core/src/memex_core/memory/retrieval/decay.py` — NEW module exporting `compute_decay_boost(unit, decay_alpha)` analogous to `compute_mw_boost` in `services/outcomes.py:45`. Closed-form `boost = 1.0 + decay_alpha × (importance × exp(-elapsed_days / stability) − 0.5)`. **NULL-handling contract**: the function early-returns `1.0` (neutral) when any of `unit.importance`, `unit.stability`, `unit.last_outcome_at` is `None` — the closed-form is only evaluated below that guard. Cold-start units (pre-F25 / pre-F11 backfill) and never-touched units compose neutrally rather than getting silently penalised by a synthetic default. Mirrors F40's branch-level `COALESCE(..., FALSE)` NULL-handling pattern (§3.4.1) — guard at the boost-result level, not the column level.
