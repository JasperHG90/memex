...
**2. New code location:**
- `packages/core/src/memex_core/memory/retrieval/decay.py` — NEW module exporting `compute_decay_boost(unit, decay_alpha)` analogous to `compute_mw_boost` in `services/outcomes.py:45`. Closed-form `boost = 1.0 + decay_alpha × (importance × exp(-elapsed_days / stability) − 0.5)`.
- `packages/core/alembic/versions/XXX_fsfm_decay_columns.py` — migration adding `importance: float`, `stability: float`, `last_outcome_at: datetime` to `MemoryUnit`. Existing rows get importance from F25's classifier output (`intent_class` → default importance map: permanent=1.0, durable=0.7, ephemeral=0.3) and stability from a per-class table (permanent=∞ via NULL handling, durable=180 days, ephemeral=14 days). `last_outcome_at` initialised to `created_at` for existing rows.
...
- `packages/core/src/memex_core/memory/retrieval/config.py` — new config knob `decay_alpha` (default 0.3, matching `mw_alpha` and `recency_alpha`) and `decay_intent_stability` map (per-class days).
...
- F40 (Tier-A latency-optimization) FSFM clause becomes active: `apply_pre_filter=True` queries can now prune long-stale low-importance units pre-reranker. F40 ships the SQL inert; F11's PR flips `fsfm_branch_enabled` to True in the same migration.
...
**2. New code location:**
- `packages/core/src/memex_core/memory/confidence.py` — NEW module with the Beta-Bernoulli posterior helpers (`mean_and_variance(alpha, beta) -> tuple[float, float]`, `sample_concentration(alpha + beta) -> float`). Pure Python, no DB I/O.
- `packages/core/alembic/versions/XXX_add_confidence_variance.py` — migration adding a `confidence_evidence_count: int` column to `MemoryUnit` (default 0). Variance is then derived inline from `(confidence, confidence_evidence_count)` via the closed-form Beta posterior — no separate `variance` column. (The existing `confidence` column at `sql_models.py:596` stays as the mean; F22 only adds the evidence-count companion.)
...
- `packages/core/src/memex_core/memory/retrieval/engine.py:1467-1474` (the F47 confidence-boost composition site) — extend the `confidence_boost` formula from `1.0 + α × (confidence − 0.5)` to attenuate by certainty: `1.0 + α × (confidence − 0.5) × certainty`, where `certainty = 1 − variance/max_variance` (zero-evidence units land near `boost = 1.0` neutral; well-evidenced units get the full boost / penalty). This is a *strict generalisation* of F47 — when `confidence_evidence_count = 0`, `certainty → 0` and the boost collapses to neutral, exactly the cold-start safety F47 already requires.
...
**2. New code location:**
- `packages/core/alembic/versions/XXX_add_mw_mode.py` — migration adding `mw_mode: text` enum (`stationary` / `ema`) to the `vaults` table (`sql_models.py:42`). Default `stationary` — no behaviour change for existing vaults.
- `packages/core/src/memex_core/memory/retrieval/mw_ema.py` — NEW module exposing `compute_mw_ema_score(success, failure, last_outcome_at, half_life_days, now)` for the EMA-decayed posterior. Closed-form: each existing counter value is decayed by `exp(-elapsed_days × ln(2) / half_life_days)` before being read; `record_outcome` continues to write integer increments but the *read path* applies decay on the fly.
