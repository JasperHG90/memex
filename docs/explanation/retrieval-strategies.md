...
**Enriched tag search:** The keyword strategy also searches enriched metadata generated by the reflection engine's Phase 6 (Memory Evolution). When reflection identifies that a memory relates to concepts not present in its original text, it writes `enriched_tags` and `enriched_keywords` into the memory's metadata. The keyword strategy includes these in the tsvector, so a memory about "auth middleware rewrite" becomes findable by "compliance" if reflection determined the rewrite is compliance-driven. This is handled via `COALESCE` on the JSONB metadata, so memories without enriched metadata are unaffected.
