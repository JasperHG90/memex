...
### Procedural memory: the procedural plane

A procedural memory is *how to do something the way this user wants it done*. The deploy steps that actually unstuck a Nomad allocation. The pull-request title style the team settled on. The sequence you reach for when you cut a release. These are not facts; they are recipes the agent follows.

Procedural memories live in the **procedural plane** — a substrate separate from notes and from the key-value store. You do not write a procedure directly. You submit *cases*: worked episodes with a trigger, the actions you took, and how it turned out. A case is itself a note with `role='case'`, filed into a hidden system vault <code-ref path="packages/core/src/memex_core/services/case_service.py" lines="3-4" />. The system distils the cases that share a `(scope, verb, context)` anchor into a derived `procedural_entries` row — a procedure — and the heuristic over the procedures sharing a `(scope, verb)` into a strategy. Procedures and strategies are derived and read-only; you read them, the system writes them.

A derived entry carries its provenance: outcome counters, source edges back to the cases that produced it, and a version ledger of every edit <code-ref path="packages/core/src/memex_core/memory/sql_models.py" lines="2088-2118" />. That is the evidence trail a stated preference does not have, and it is why procedures live on their own plane rather than as a string in the KV store.

Concrete example. You finish a deploy that hung on a stuck Nomad allocation and only cleared after a drain-wait-resubmit. You submit a case — trigger "Nomad deploy hangs on stuck allocations", actions describing the fix, outcome `success` — through `memex_case_submit` on the MCP surface or `memex case submit` on the CLI. No procedure matches the trigger cleanly, so the system files a draft procedure anchored on `(global, deploy, nomad)` and attaches the case. Once a reviewer activates it, the next "deploy is stuck on Nomad" search surfaces the exact steps that worked. The recipe was *learned from doing the task*, not stated.

A stated one-line convention is a different shape and goes elsewhere. "Always lint before you commit" is a rule the user expressed in words, not a recipe distilled from episodes — that is a convention, and conventions live in the KV store (see the cross-context section below). The line between them is provenance: a convention is true because the user said so; a procedure is true because the system watched it work.
...
Entities are global. The same `Alice` is one entity in your personal vault and in your work vault; the same `ruff` is one entity in every Python project you track <code-ref path="packages/core/src/memex_core/memory/sql_models.py" lines="933-1036" />. Cross-vault lineage queries follow this: `memex_get_lineage` walks from a mental model down through observations and memory units to the source note, crossing vault boundaries when the entity is shared <code-ref path="packages/mcp/src/memex_mcp/server.py" lines="2905-2926" />.

The KV store is also cross-vault. Keys carry a namespace prefix — `global:`, `user:`, `project:`, or `app:` — and the service enforces that one of those four prefixes leads every key <code-ref path="packages/core/src/memex_core/services/kv.py" lines="58-67" />. A `global:lang:python:min` key applies everywhere; a `project:github.com/user/repo:vault` key applies only to that project, regardless of which vault is active. The four prefixes are the only namespaces KV recognises — there is no `procedure:` namespace, and procedural knowledge does not live in KV at all (it lives on the procedural plane above). Pick the namespace by the scope of the rule, not by the vault you happen to be in.
...
**Procedural memory lives on its own plane, not in notes or KV.** A recipe could be stored as a note — "here's how I write PR titles for this project" — and retrieved by semantic search. The reason it does not is access pattern. A procedure has to come back as the *one* recipe that fits the task, with the evidence behind it; semantic search over notes would sometimes miss, sometimes return three competing recipes, sometimes return a stale one. The procedural plane gives a procedure its own row, derived from the cases that produced it, with outcome counters and source edges. The trade is that you give up the simplicity of a single note — but a note cannot carry the provenance a learned recipe needs.

A stored *convention* is the lighter cousin and lives in KV. "Always lint before you commit" is one static binding the user stated in words, retrieved by exact key. That is the KV path: a fast key lookup with a single active value. The line is provenance, not just access pattern — a convention is a rule you were given; a procedure is a recipe the system learned by watching the task succeed.

The same logic explains why semantic memories do *not* live in KV. A fact like "ruff 0.4 dropped support for Python 3.8" has no natural key the agent will know to look up; it has to be discovered by similarity or by walking the entity graph. Putting it in KV would force the writer to invent a key the reader will never guess. The pairing of substrates is deliberate: facts go where similarity search is the entry point; conventions go where exact lookup is; recipes go on the plane where provenance is. See [the procedural memory plane](procedural-memory.md) for the full model.
...
**Is the user asking the agent to do something a particular way?** That is procedural. If it is a recipe worked out by doing a task, submit a case and let the procedural plane derive the procedure; recall it later with `memex_procedural_search`. If it is a one-line convention the user simply stated ("always lint before you commit"), store it in KV under an ordinary descriptive key. Either way, do not write a note describing the procedure — semantic search over notes will not reliably retrieve it at the moment the verb fires.
...
**"What does deploy mean for this project — staging or production?"** The agent is about to *do* something — run a deploy verb — and it needs the recipe for that verb in this project. That is procedural. The agent calls `memex_procedural_search("deploy this project")` and follows the derived procedure anchored on `(project:<id>, deploy, ...)`. If no procedure matches, the agent asks; once it has run the deploy, it submits a case so the plane learns the recipe for next time.

**"How do I usually write a pull-request title in this repo?"** Same shape — a verb (`write_pr`) adapted to a context (the repo). The agent searches the procedural plane for the derived `write_pr` procedure scoped to the repo. The recipe comes back with the cases that back it, or it is absent — a signal to capture one once the agent works out the convention.
