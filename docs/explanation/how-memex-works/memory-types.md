...
### Procedural memory: KV under the `procedure:` namespace
...
Procedural memories live in the key-value store under keys shaped like `procedure:<verb>:<context-tag>`. The key format is enforced by a strict regex — exactly two colons, lowercase identifiers, no slashes <code-ref path="packages/core/src/memex_core/services/kv.py" lines="46-77" />. The agent owns the verb (`write_pr`, `run_tests`, `deploy`); Memex stores observations about how to *adapt* that verb to the named context.
...
Concrete example. The user says: "for this project, when you write a PR title, lead with the package name in square brackets." The agent writes `procedure:write_pr:memex-monorepo` with the recipe as the value (calling `memex_kv_put` on the MCP surface or `memex kv put` on the CLI <code-ref path="packages/mcp/src/memex_mcp/server.py" lines="3275-3306" />). Three weeks later the user revises the style. The agent writes the new value to the same key; the old value falls into the row's history list, the version bumps to 2, and the next session reads the new convention with the previous one preserved for context.
...
The KV store is also cross-vault. Keys carry a namespace prefix — `global:`, `user:`, `project:`, `app:`, `procedure:` — and the service enforces that prefix on every write <code-ref path="packages/core/src/memex_core/services/kv.py" lines="44-100" />. A `global:lang:python:min` key applies everywhere; a `project:github.com/user/repo:vault` key applies only to that project, regardless of which vault is active. Pick the namespace by the scope of the rule, not by the vault you happen to be in.
...
**Is the user asking the agent to do something a particular way?** That is procedural. Use the KV store under `procedure:<verb>:<context-tag>`. Do not write a note describing the procedure — semantic search will not reliably retrieve it at the moment the verb fires, and the recipe will drift away from the verb it adapts.
...
**"What does deploy mean for this project — staging or production?"** The agent is about to *do* something — run a deploy verb — and it needs the user's adaptation of that verb to this project. That is procedural. The agent reads `procedure:deploy:<project-tag>` directly, gets the active value, and proceeds. If the key is missing, the agent asks; if the key has a five-entry history showing a recent revision, the agent uses the active value and surfaces the change if the user seems unsure.

**"How do I usually write a pull-request title in this repo?"** Same shape — a verb (`write_pr`) adapted to a context (the repo). The agent reads `procedure:write_pr:<repo-tag>`. The retrieval is one primary-key lookup; the recipe is right or it is absent, never almost-right.
