...
Each strategy applies a shared **scope filter** before returning. The filter keeps only active units (`status = 'active'`) and non-deprioritized units (`is_deprioritized = false`) by default; callers can opt in to stale or deprioritized units via `include_stale` and `include_deprioritized`. The filter is centralized in one function, `apply_generic_filters`, called from every strategy's SQL build so a configuration change can't slip past one strategy. <code-ref path="packages/core/src/memex_core/memory/retrieval/strategies.py" lines="113-132" />
...
A defensive guard catches NaN and `±inf` inputs — if any boost is non-finite, the chain short-circuits and returns the bare `ce_score`, incrementing a Prometheus counter so the upstream misbehaviour surfaces. <code-ref path="packages/core/src/memex_core/memory/retrieval/engine.py" lines="171-177" />
...
- *Temporal extraction returns nothing.* The query runs without a date filter. If the regex extracted a date range but the filtered query returned zero results, the engine retries without the temporal constraint. <code-ref path="packages/core/src/memex_core/memory/retrieval/engine.py" lines="709-712" />
...
- *Reranker unavailable.* The reranker step is where both the cross-encoder reweight and the composition chain run, so when no reranker is loaded the engine returns the RRF-fused results unchanged. Quality degrades but the pipeline doesn't crash. <code-ref path="packages/core/src/memex_core/memory/retrieval/engine.py" lines="1718-1723" />
- *A boost factor produces a non-finite value.* The composition chain's defensive guard short-circuits to bare `ce_score` for that unit and increments a Prometheus counter so the upstream bug surfaces. <code-ref path="packages/core/src/memex_core/memory/retrieval/engine.py" lines="171-177" />
