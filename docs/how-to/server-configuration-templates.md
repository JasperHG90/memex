...
| `composite_boost_log_clip` | `inf` | Eventually | Symmetric log-space clip on the aggregate metadata multiplier (recency × temporal × MW × confidence × decay). At default `inf` the clip is a no-op (mathematically identical to the prior product for strictly positive boost inputs). Set to a finite `L` (e.g. `0.7` → aggregate bounded to roughly `[0.50, 2.01]`; `1.0` → `[0.37, 2.72]`; `1.5` → `[0.22, 4.48]`) once the `memex_composite_boost_clipped` histogram has accumulated representative traffic — at `L=inf` this metric observes the pre-clip product directly. See [Retrieval scoring](../explanation/retrieval-scoring.md). |
