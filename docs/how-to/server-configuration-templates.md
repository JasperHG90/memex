...
| `reranking_recency_alpha` | 0.2 | Rarely | Recency boost strength during reranking. Composed log-additively with the other four boost factors (see `composite_boost_log_clip`). 0 = disabled, 0.2 = subtle, 0.5 = strong. |
| `composite_boost_log_clip` | `inf` | Eventually | Symmetric log-space clip on the aggregate metadata multiplier (recency × temporal × MW × confidence × decay). At default `inf` the clip is a no-op (mathematically identical to the prior product for strictly positive boost inputs). Set to a finite `L` (e.g. `0.7` → aggregate bounded to roughly `[0.50, 2.01]`; `1.0` → `[0.37, 2.72]`; `1.5` → `[0.22, 4.48]`) once the `memex_composite_boost_clipped` histogram has accumulated representative traffic — at `L=inf` this metric observes the pre-clip product directly. See [Retrieval scoring](../explanation/retrieval-scoring.md) for the design rationale and [Observability](../reference/observability.md#reranking-composition-metrics) for the tuning workflow. |
