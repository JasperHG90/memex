...
- **Append, don't re-send the full body**: To extend a note you already created (session log, ongoing reflection), call `memex_append_note(note_key=..., delta=...)` rather than `memex_add_note`. Append sends only the delta, runs incremental extraction (only new chunks invoke the LLM), and is atomic + idempotent (caller-supplied `append_id`).
