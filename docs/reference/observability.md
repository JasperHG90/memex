...
4. Re-run the retrieval-ranking regression evaluation under the `memex-eval` CLI to confirm ranking drift stays within the deployment's quality budget. `memex-eval suite list` enumerates the built-in suites; the `acme_corp` suite covers per-factor reranking knobs (`reranking_mw_alpha`, `reranking_recency_alpha`, `reranking_temporal_alpha`) and can be swept against `composite_boost_log_clip` via `--override` (run it with `memex-eval suite run acme_corp`). The LoCoMo external benchmark is a five-phase pipeline (`ingest → export → answer → judge → report`) whose canonical command sequence and required `--questions` / `--answers` / `--output` / `--results` filename wiring live in `packages/eval/README.md` — copy that exact sequence rather than chaining subcommands with defaults, which fail on a filename mismatch. LoCoMo dataset acquisition is the deployment's responsibility (the README assumes the corpus is already in `./data/locomo/`). The `pytest -m benchmark` retrieval suite measures composition wall-clock latency only — it is not a ranking-quality signal.
