...
4. Re-run the retrieval-ranking regression evaluation under the `memex-eval` CLI to confirm ranking drift stays within the deployment's quality budget. `memex-eval suite list` enumerates the built-in suites; `acme_corp` exercises per-factor reranking knobs (`reranking_mw_alpha`, `reranking_recency_alpha`, `reranking_temporal_alpha`). The correct workflow for comparing different `composite_boost_log_clip` values is: (a) set `composite_boost_log_clip` in server YAML or via the `MEMEX_SERVER__MEMORY__RETRIEVAL__COMPOSITE_BOOST_LOG_CLIP` env var, (b) restart the server so the new value is live, (c) `memex-eval suite run acme_corp --override server.memory.retrieval.composite_boost_log_clip=0.7` — the `--override` is logged to MLflow only and tags the run for later comparison; it does NOT reconfigure a running server (the CLI prints a warning to this effect). Repeat for each value to compare. The LoCoMo external benchmark is the deeper external signal; `packages/eval/README.md` shows its example invocation chain (export → answer → judge → report), to which `locomo ingest` is the prerequisite phase that loads the corpus. Copy that explicit sequence rather than chaining subcommands with defaults, which fail on a filename mismatch. LoCoMo dataset acquisition is the deployment's responsibility; pass the corpus location to `--dataset-path`. The `pytest -m benchmark` retrieval suite measures composition wall-clock latency only — it is not a ranking-quality signal.
