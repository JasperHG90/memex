...
4. Re-run the retrieval-ranking regression evaluation under the `memex-eval` CLI to confirm ranking drift stays within the deployment's quality budget. `memex-eval suite list` enumerates the built-in suites; the `acme_corp` suite exercises reranking-composition knobs directly. The LoCoMo external benchmark is a four-phase pipeline (`ingest → export → answer → judge → report`) whose canonical invocation, prerequisite dataset setup, and required `--questions` / `--answers` / `--output` / `--results` filename wiring live in `packages/eval/README.md` — copy that exact command sequence rather than chaining subcommands with defaults, which fail on a filename mismatch. The `pytest -m benchmark` retrieval suite measures composition wall-clock latency only — it is not a ranking-quality signal.
