...
3. Set `composite_boost_log_clip = L` via the standard configuration path (server YAML or the `MEMEX_SERVER__MEMORY__RETRIEVAL__COMPOSITE_BOOST_LOG_CLIP` env var) and restart the server so the new value takes effect — Memex does not currently hot-reload retrieval config.
4. Re-run the retrieval-ranking regression evaluation under the `memex-eval` CLI to confirm ranking drift stays within the deployment's quality budget. `memex-eval suite list` enumerates the built-in suites; `acme_corp` exercises per-factor reranking knobs (`reranking_mw_alpha`, `reranking_recency_alpha`, `reranking_temporal_alpha`). To compare different `composite_boost_log_clip` values: restart the server once per value (step 3), then for each value run `memex-eval suite run acme_corp --override server.memory.retrieval.composite_boost_log_clip=0.7`. The `--override` is logged to MLflow only — it tags each run for later comparison but does NOT reconfigure a running server (the CLI prints a warning to this effect).

   The LoCoMo external benchmark is the deeper external signal. Its pipeline is `ingest → export → answer → judge → report`; `ingest` is a one-time setup that loads the corpus into Memex (the deployment supplies the LoCoMo dataset and passes its path to `--dataset-path`), and the export → answer → judge → report cycle runs per evaluation. See `packages/eval/README.md` for the example invocation chain; copy its explicit `--questions` / `--answers` / `--output` / `--results` filename wiring rather than chaining subcommands with defaults, which fail on a filename mismatch. The `pytest -m benchmark` retrieval suite measures composition wall-clock latency only — it is not a ranking-quality signal.
