...
| `memex_get_page_index` | Get the hierarchical page index (table of contents) for a document. | `note_id` |
...
| `memex_add_note` | Save a new note with optional attachments. | `title`, `markdown_content`, `description`, `author`, `tags`, `supporting_files`, `vault_id`, `document_key`, `background` |
| `memex_get_template` | Get a markdown template for note creation (e.g., ADR). | `type` |
| `memex_list_assets` | List all files (images, PDFs) attached to a document. | `note_id` |
