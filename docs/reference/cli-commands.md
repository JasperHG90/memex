...
Manage the Memex web dashboard (React + Vite).

### `dashboard start`

```
memex dashboard start [OPTIONS]
```

Start the Memex Dashboard.

* **Development mode** (`--dev`): Runs `npm run dev` with hot module replacement.
* **Production mode** (default): Serves the pre-built `dist/` directory using `npx serve`.
...
| `--dev` | | bool | `False` | Run in development mode (hot reload via Vite). |
| `--daemon` | `-d` | bool | `False` | Run in the background (production mode only). |

#### Prerequisites

* Node.js must be installed and on `PATH`.
* For production mode: run `npm run build` in the dashboard directory first.
...
# Start production build as a daemon
memex dashboard start --daemon

# Start on a custom port
memex dashboard start --port 3001
```

> [!WARNING]
> `--daemon` is not supported with `--dev`. The daemon flag is ignored in development mode.
...
```
memex dashboard stop
```

Stop the running Memex Dashboard. Sends SIGTERM to the entire process group.
...
```
memex dashboard status
```

Check the status of the Memex Dashboard. Verifies both the process (via PID file) and HTTP health (via root URL).
