...
- [ ] **F11** — FSFM-lite decay scoring (Ebbinghaus × importance, composed at the reranker) (3-4w, promoted 2026-05-02) — see report §4 F11 + §3.4 (composition site) + §3.4.1 (KinthAI principle / cold-start neutrality). Trigger formerly: reflection cron / lived experience shows stale-bias; now elevated to Tier A because F40's FSFM pre-filter clause depends on F11's columns to activate. **Concrete scope:**
  ...
  - Flip F40's `fsfm_branch_enabled: bool = False` to `True` *in the same PR* so F40's pre-reranker FSFM clause activates only after F11's columns exist.
  ...
  - **Acceptance**: before/after benchmark shows quality lift on long-tail-content queries with `decay_alpha = 0.3`; cold-start units stay neutral; F40's FSFM branch fires on stale + low-importance units in the test vault; all existing reranker tests pass with `decay_alpha = 0.0` (composition is a no-op at the default).
...
- [ ] **F22** — Two-Factor edge confidence (variance alongside mean) (3-4w, promoted 2026-05-02) — see report §4 F22 + ZenBrain §B.1 + Memory Worth (arXiv:2604.12007v1) §3.4 "Two Counts Are Necessary" precedent. **Concrete scope:**
  ...
  - New module `packages/core/src/memex_core/memory/confidence.py` exposes `mean_and_variance(alpha, beta) -> tuple[float, float]` and `sample_concentration(alpha + beta) -> float`. Pure Python, no DB I/O.
  ...
  - **Test plan**: cold-start unit (`confidence_evidence_count = 0`) → `certainty = 0` → `confidence_boost = 1.0` (neutral) — regression guard for F47 cold-start safety; well-evidenced contradicted unit (`confidence = 0.5, evidence_count = 20`) → low variance, full F47 penalty applies; well-evidenced clean unit (`confidence = 1.0, evidence_count = 20`) → low variance, full F47 lift applies; lint-finding gating test exercises the `(confidence, variance)` joint condition.
...
- [ ] **F35** — Non-stationary EMA mode for MW counters (1-2w, promoted 2026-05-02) — see report §4 F35 + Memory Worth (arXiv:2604.12007v1) §6 + §3.4.2 Theorem 4.1 caveat. Trigger formerly: MW drift observed; promoted to Tier A because the lever is cheap and complements F1c / F33. **Concrete scope:**
  ...
  - New module `packages/core/src/memex_core/memory/retrieval/mw_ema.py` exposes `compute_mw_ema_score(success, failure, last_outcome_at, half_life_days, now)` for the EMA-decayed posterior. Closed-form: each counter value is decayed by `exp(-elapsed_days × ln(2) / half_life_days)` before being read; `record_outcome` continues to write integer increments — only the read path applies decay.
