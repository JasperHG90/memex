...
- [ ] **F22** — Two-Factor edge confidence (variance alongside mean) (3-4w, promoted 2026-05-02) — see report §4 F22 + ZenBrain §B.1 + Memory Worth (arXiv:2604.12007v1) §3.4 "Two Counts Are Necessary" precedent. **Concrete scope:**
  ...
  - **Acceptance**: before/after benchmark shows the F47 reranker boost matches expected behaviour at both cold-start (neutral) and high-evidence (full multiplier) extremes; F6 / F10 false-positive lint findings drop on a representative ingestion workload; reflection cron's per-tick LLM budget concentrates on high-variance edges (observable in metrics).
