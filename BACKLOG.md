...
- [ ] **F10b** — Polarity-discriminating NLI classifier on top of F10's surprise gate (2-3w, promoted 2026-05-02) — see report §4 F10b + `packages/core/src/memex_core/memory/lint_llm/signatures.py:15-19` (the deferred-from-F10 docstring). Trigger: ≥10 false-contradiction findings/month in production. **Concrete scope:**
...
- [ ] **F11** — FSFM-lite decay scoring (Ebbinghaus × importance, composed at the reranker) (3-4w, promoted 2026-05-02) — see report §4 F11 + §3.4 (composition site) + §3.4.1 (KinthAI principle / cold-start neutrality). Trigger formerly: reflection cron / lived experience shows stale-bias; now elevated to Tier A because F40's FSFM pre-filter clause depends on F11's columns to activate. **Concrete scope:**
  ...
  - **NULL-handling contract (load-bearing)**: `compute_decay_boost` MUST early-return `1.0` (neutral) if any of `unit.importance`, `unit.stability`, `unit.last_outcome_at` is `None`. The first line of the function is `if unit.importance is None or unit.stability is None or unit.last_outcome_at is None: return 1.0`. The closed-form expression is only evaluated below that guard. Rationale: cold-start units (pre-F25 / pre-F11 backfill) and units that have never had an outcome recorded (`last_outcome_at IS NULL`) must compose neutrally — they are *not* "high decay" because we have no evidence; they are "no signal yet". Implementations MUST NOT substitute synthetic defaults (e.g., `importance = 0.5`, `stability = 30 days`) — that would silently boost or penalise unclassified units rather than leave them neutral. The guard mirrors F40's branch-level `COALESCE(..., FALSE)` pattern (BACKLOG.md F40 NULL-handling commitment) — guard at the *boost-result* level, not at the column level.
  - Migration `packages/core/alembic/versions/XXX_fsfm_decay_columns.py` adds `importance: float`, `stability: float`, `last_outcome_at: datetime` to `MemoryUnit` (`sql_models.py:566-636`). Backfill: importance from F25's `intent_class` map (permanent=1.0, durable=0.7, ephemeral=0.3); stability from per-class table (permanent=∞ via NULL, durable=180 days, ephemeral=14 days); `last_outcome_at` initialised to `created_at`.
  - Constants live in a new `memex_core.memory.retrieval.constants` module so F40's FSFM SQL clause (BACKLOG.md F40) and F11's Python boost share `STABILITY_SECONDS_PER_DAY = 86400` and the intent-stability map. F40 imports the constant; F11 imports the same constant — single source of truth.
  ...
  - Flip F40's `fsfm_branch_enabled: bool = False` to `True` *in the same PR* so F40's pre-reranker FSFM clause activates only after F11's columns exist. **Rollback decoupling — three independent kill-switches in the same PR**:
    ...
    3. `record_outcome` no longer writing `last_outcome_at` (revert the `outcomes.py:65` extension) → the column persists with `created_at`-equivalent timestamps; nothing breaks.
  ...
  - **Test plan**: cold-start (`importance=NULL` AND `stability=NULL` AND `last_outcome_at=NULL`) → `decay_boost = 1.0` (neutral); recently-touched durable unit → `decay_boost ≈ 1.06` (mild lift); 3-year-old ephemeral unit → `decay_boost ≈ 0.85` (penalty); `decay_alpha = 0.0` config → composition is a no-op (regression guard).
...
- [ ] **F22** — Two-Factor edge confidence (variance alongside mean) (3-4w, promoted 2026-05-02) — see report §4 F22 + ZenBrain §B.1 + Memory Worth (arXiv:2604.12007v1) §3.4 "Two Counts Are Necessary" precedent. **Concrete scope:**
  ...
  - **Backfill strategy for existing rows**: the migration inspects the existing `confidence` value and counts incoming `MemoryLink` rows of type `contradicts` / `weakens` / `reinforces` (per `packages/core/src/memex_core/memory/contradiction/engine.py:225-227` link convention) to backfill `confidence_evidence_count` non-destructively: for each unit, set `confidence_evidence_count = COUNT(MemoryLink WHERE to_unit_id = unit.id AND link_type IN ('contradicts', 'weakens', 'reinforces'))`. Units with no contradiction history end up at `0` (truly never observed); units with prior contradictions get a non-zero count proportional to the evidence the contradiction engine has already accumulated. Combined with the ship-time guard below, this gives the column a meaningful starting state without mutating semantics — pre-F22 vaults that have already been running F47 see their certainty derive from the contradiction events that produced the current `confidence` value.
  ...
  - Extend `packages/core/src/memex_core/memory/contradiction/engine.py:211-219` (the existing `α=0.1` weaken / `2α=0.2` contradict steps) to bump `confidence_evidence_count` by 1 on every step. Link-construction at `engine.py:225-227` is untouched.
...
- [ ] **F35** — Non-stationary EMA mode for MW counters (1-2w, promoted 2026-05-02) — see report §4 F35 + Memory Worth (arXiv:2604.12007v1) §6 + §3.4.2 Theorem 4.1 caveat. Trigger formerly: MW drift observed; promoted to Tier A because the lever is cheap and complements F1c / F33. **Concrete scope:**
  - Migration `packages/core/alembic/versions/XXX_add_mw_mode.py` adds `mw_mode: text` enum (`stationary` / `ema`, default `stationary`) to the `vaults` table (`packages/core/src/memex_core/memory/sql_models.py:42`). Vault-scoped, not global, so different vaults can opt in independently.
