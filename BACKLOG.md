...
- [ ] **F41** — Cross-encoder score cache (1-2w) — see report §3.4.1 (latency levers list). Keyed on `(query_embedding_hash, unit_id)` with 24h TTL. Repeat queries (e.g., daily morning briefing loop, recurring user templates) get free reranking. Biggest single lever for narrow query patterns; effectiveness depends on workload. Independent of F40 — they compose. Add cache-hit-rate metric to validate impact.
  - **Cache invalidation on model change**: the `(query_embedding_hash, unit_id)` key encodes only the query embedding, not the embedding-model identity. Any change to the embedding model (upgrade, quantization, dimension change) MUST flush the cache or cycle the key namespace — stale entries produce reranker scores that don't match the current model. Track this invariant in the F41 PR's deploy-runbook.
